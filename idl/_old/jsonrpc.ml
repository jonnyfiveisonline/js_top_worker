...
let rec json_to_rpc t =
  ...
  | `Assoc a ->
      Dict (Rpcmarshal.tailrec_map (fun (k, v) -> (k, json_to_rpc v)) a)
...
let string_of_call ?(version = V1) call =
  ...
  in
  let json =
    if not call.is_notification then json @ [ ("id", Int (new_id ())) ]
    else json
  in
  to_string (Dict json)
...
let json_of_response ?(id = Int 0L) version response =
  if response.Rpc.success then
    match version with
    | V1 ->
        Dict [ ("result", response.Rpc.contents); ("error", Null); ("id", id) ]
    | V2 ->
        Dict
          [
            ("jsonrpc", String "2.0");
    ...
    match version with
    | V1 ->
        Dict [ ("result", Null); ("error", response.Rpc.contents); ("id", id) ]
    | V2 ->
        Dict
          [
            ("jsonrpc", String "2.0");
...
let version_id_and_call_of_string_option str =
          ...
          | _ ->
              raise
                (Malformed_method_request
                   "Invalid field 'method' in request body")
        in
        let version =
          ...
          | _ ->
              raise
                (Malformed_method_request
                   "Invalid field 'jsonrpc' in request body")
        in
        let params =
          match version with
          | V1 -> (
  ...
  | Missing_field field ->
      raise
        (Malformed_method_request
           (Printf.sprintf "Required field %s is missing" field))
  | JsonToRpcError json ->
      raise
        (Malformed_method_request
...
let version_id_and_call_of_string s =
  ...
  | None ->
      raise (Malformed_method_request "Invalid field 'id' in request body")
...
let get_response extractor str =
            ...
            | x, y ->
                raise
                  (Malformed_method_response
                     (Printf.sprintf "<result=%s><error=%s>" (Rpc.to_string x)
                        (Rpc.to_string y))))
        | Some (String "2.0") -> (
            let result = get' "result" d in
            let error = get' "error" d in
  ...
  | Missing_field field ->
      raise
        (Malformed_method_response (Printf.sprintf "<%s was not found>" field))
  | JsonToRpcError json ->
      raise
        (Malformed_method_response
           (Printf.sprintf "<unable to parse %s>" (Y.to_string json)))
